...
bool Application::finishUpdate() {
	...
	static char title[256];
	sprintf_s(title, 256, "Average FPS: %.2f, Frames on last update: %d, Particles use: %d ms, %d Particles count:", avg_fps, frames_on_last_update, App->particle_system->time_to_update, App->particle_system->particles_count);
	App->window->setWindowTitle(title);
